...
class Dir {
	constructor(filename) {
		...
		filename = FS_ShrinkPath(filename)
...
function SelectFile(callback, CWD="", opt={}) {
...
// example of file selector
// after uses selects file, they are redirected
// to it's /files/ endPoint
//window.onload = async () => {
//	await FS_REMIRROR();
...
const FS_default_draw_options = {
	confirmSelect: false, //TODO: implement double-click confirmation
	selectFile:true, //TODO: implement not selecting files
	selectFolder:false, // TODO: implement folder selection/walking
	createFile:false, // TODO: implement server-mirroring
	createFolder:false, // TODO: implement server-mirroring
}
...
function FS_redraw(
	...
	FS_walk(FS, CWD)
		...
		tbody.appendChild( FS_TROW(folder, ()=>{
			FS_redraw(Dialog, FS, CWD+"/"+folder.fullname, callback, options)
		}))
	})
...
function MakeFile(fsEndpoint, name, content="") {
	console.error(`Make File ${name} @ ${fsEndpoint} "${content}" `)
}

function MakeFolder(fsEndpoint, name) {
	console.error(`Make Folder ${name} @ ${fsEndpoint}`)
}

function LoadFile(filesEndpoint) {
	return fetch(filesEndpoint, {cache: "no-cache"}).then(a=>a.text())
}

function LoadJson(filesEndpoint) {
	return fetch(filesEndpoint, {cache: "no-cache"}).then(a=>a.json())
}
